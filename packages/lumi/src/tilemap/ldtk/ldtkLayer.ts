...
export class LdtkLayer {
  ...
  updateVisibleTiles(bounds: Rectangle): void {
    ...
    tlX -= 1;
    tlY -= 1;
    tlX = LumiMath.clamp(tlX, 0, this.getWidth());
    tlY = LumiMath.clamp(tlY, 0, this.getHeight());

    let [brX, brY] = this.pixelToTilePosition(bounds.x + bounds.width, bounds.y + bounds.height);
    brX += 2;
    brY += 2;
    brX = LumiMath.clamp(brX, 0, this.getWidth());
    brY = LumiMath.clamp(brY, 0, this.getHeight());
