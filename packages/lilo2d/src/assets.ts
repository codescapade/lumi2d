...
export class Assets {
  ...
  static loadAtlas(name: string, imagePath: string, dataPath: string, keep = true): Atlas {
    const image = Assets.loadImage(`lilo_atlas_${name}`, imagePath, keep);
    const data = Assets.loadText(`lilo_atlas_${name}`, dataPath, keep);
  ...
  static removeAtlas(name: string): void {
    ...
    Assets.removeImage(`lilo_atlas_${name}`);
    Assets.removeText(`lilo_atlas_${name}`);
  }
